...
//doubles the size
void Particle::enlarge(){
	scale *= 2;
}

//halves the size
void Particle::shrink(){
	scale /= 2;
}

//doubles the velocity
void Particle::faster(){
	vel.x *= 2;
	vel.y *= 2;
}

//halves the velocity
void Particle::slower(){
	vel.x /= 2;
	vel.y /= 2;
}
...
void Particle::draw(){
	...
	//display starts with circles
	if(circle){
	ofDrawCircle(pos.x, pos.y, scale * 4.0);
	//draws smaller circle white within the previous circle
	ofSetColor(235);
	ofDrawCircle(pos.x, pos.y, scale * 2.0);
	}
	// if key pressed, changes shape to triangle
	else if(triangle){
	ofDrawTriangle(pos.x, pos.y, pos.x-10*scale, pos.y+10*scale, pos.x+10*scale, pos.y+10*scale);
	}
	//if key pressed again, changes shape to square
	else{
		ofDrawRectangle(pos.x, pos.y, 10*scale, 10*scale);
	}
}
